...
namespace Gfx
{
	...
	bool ReadSentences(const CBuffer& _kSrc, CSentences* _pkSentences)
	{
		wchar_t aszRedirect[256] = { 0 };
		aszRedirect[Uint8('|')]		= L'\x00e9'; // é
		aszRedirect[Uint8('}')]		= L'\x00e8'; // è
		aszRedirect[Uint8('\\')]	= L'\x00ea'; // ê
		aszRedirect[Uint8('~')]		= L'\x00f9'; // ù
		aszRedirect[93]				= L'\x00ee'; // î
		aszRedirect[95]				= L'\x00fb'; // û
		aszRedirect[91]				= L'\x00e2'; // â
		aszRedirect[123]			= L'\x00e0'; // à
		aszRedirect[127]			= L'\x00e7'; // ç

		const Uint8* pcData = _kSrc.Data();

		Uint32 iSentenceSize = GetOffset(pcData, 0);
		Uint32 iSentenceCount = iSentenceSize / 2 - 8;
		
		_pkSentences->Release();
		_pkSentences->Init(iSentenceCount);
		...
		{
			Uint32 iStart = GetOffset(pcOffsetArray, i);
			Uint32 iEnd = GetOffset(pcOffsetArray, i + 1);
			
			Uint32 iCharCount = iEnd - iStart;
			for (Uint32 j = 0; j < iCharCount - 1; ++j)
			{
				Uint8 iChar = pcData[iStart + j];
				wchar_t iWChar;
				if (aszRedirect[iChar] != 0)
					iWChar = aszRedirect[Uint8(iChar)];
				else
					iWChar = btowc(iChar);
				szBuffer[j] = iWChar;
			}
			szBuffer[iCharCount] = L'\0';
			
			_pkSentences->Set(i, szBuffer);

			size_t iMbSize = wcstombs(szPrintf, szBuffer, iCharCount);
			printf("%03i -> '%s'\n", i, szPrintf);
		}
